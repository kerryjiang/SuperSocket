...
namespace SuperSocket.Tests
{
    public class SequenceReaderExtensionTest
    {
        ...
        [Fact]
        public void TestReadLittleEndianUShort()
        {
            var arr = new byte[4];
            Span<byte> span = arr;
            ...
            BinaryPrimitives.WriteUInt16LittleEndian(span, value1);
            BinaryPrimitives.WriteUInt16LittleEndian(span.Slice(2), value2);

            var sequence = new ReadOnlySequence<byte>(arr);            
            var reader = new SequenceReader<byte>(sequence);

            Assert.True(reader.TryReadLittleEndian(out ushort result1));
            Assert.Equal(value1, result1);

            Assert.True(reader.TryReadLittleEndian(out ushort result2));
            Assert.Equal(value2, result2);
        }
        ...
        public void TestReadLittleEndianUInt()
        {
            ...
            BinaryPrimitives.WriteUInt32LittleEndian(span, value1);
            BinaryPrimitives.WriteUInt32LittleEndian(span.Slice(4), value2);

            var sequence = new ReadOnlySequence<byte>(arr);            
            var reader = new SequenceReader<byte>(sequence);

            Assert.True(reader.TryReadLittleEndian(out uint result1));
            Assert.Equal(value1, result1);

            Assert.True(reader.TryReadLittleEndian(out uint result2));
            Assert.Equal(value2, result2);
        }

        [Fact]
        public void TestReadLittleEndianULong()
        {
            var arr = new byte[16];
            Span<byte> span = arr;
            ...
            BinaryPrimitives.WriteUInt64LittleEndian(span, value1);
            BinaryPrimitives.WriteUInt64LittleEndian(span.Slice(8), value2);

            var sequence = new ReadOnlySequence<byte>(arr);            
            var reader = new SequenceReader<byte>(sequence);

            Assert.True(reader.TryReadLittleEndian(out ulong result1));
            Assert.Equal(value1, result1);

            Assert.True(reader.TryReadLittleEndian(out ulong result2));
            Assert.Equal(value2, result2);
        }
    }
}