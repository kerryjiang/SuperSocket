...
{
    public class HttpBodyReceiveFilter : FixedSizeReceiveFilter<HttpRequestInfo>
    {
        ...
        public override HttpRequestInfo ResolvePackage(IList<ArraySegment<byte>> packageData)
        {
            var body = Encoding.UTF8.GetString(packageData, m_HeaderSize, packageData.Sum(d => d.Count) - m_HeaderSize);
            return new HttpRequestInfo(m_Header.Path, m_Header, body);
        }
    }
}
